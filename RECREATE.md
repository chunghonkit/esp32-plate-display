...
3. Once on WiFi → connect MQTT → subscribe to topic → render incoming plate text — stays visible until the next detection replaces it. Status LED dot: green = connected, light grey = disconnected. Detection timestamp (DD/MM HH:MM:SS) shown bottom-left, HK time via SNTP (TZ `HKT-8`, `pool.ntp.org`).
