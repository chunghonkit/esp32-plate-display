...
3. Once on WiFi → connect MQTT → subscribe to topic → render incoming plate text — holds ~20 s then clears to `---`; a new plate refreshes the window. Status LED dot: green = connected, light grey = disconnected.
